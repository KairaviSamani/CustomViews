...
public class CustomLayout extends RelativeLayout implements CustomViews {
    ...
    public void processAttributes(final Context context, final AttributeSet attrs) {
        TypedArray attrsArray = context.obtainStyledAttributes(attrs, R.styleable.CustomLayout, 0, 0);
    ...
    public void initAttributes(TypedArray attrs) {
        try {
            radius = attrs.getDimension(R.styleable.CustomLayout_custom_radius, radius);
            borderColor = attrs.getColor(R.styleable.CustomLayout_custom_borderColor, borderColor);
            borderWidth = (int) attrs.getDimension(R.styleable.CustomLayout_custom_borderWidth, borderWidth);
            backgroundImage = attrs.getDrawable(R.styleable.CustomLayout_custom_background);
            backgroundColor = attrs.getColor(R.styleable.CustomLayout_custom_background, backgroundColor);
        } catch (Exception e){
            e.printStackTrace();
        }
    }
